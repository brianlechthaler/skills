...
- [ ] Storage devices identified — type (NVMe/SSD/HDD), mount points, capacity (`lsblk`, `df -h`)
- [ ] GPU vendor and model identified (or confirmed absent)
- [ ] Workload command, duration, and concurrency documented
- [ ] Monitoring tools verified (`sensors`, `nvidia-smi`, `htop`, `iostat`, etc.)
- [ ] Primary metric of interest stated (peak GPU%, RAM, CPU temp, disk `%util`, etc.)
...
- [ ] Disk throughput, IOPS, `%util`, and `await` recorded (or N/A with evidence)
- [ ] Disk space used / available recorded (`df -h`)
...
- [ ] Disk throughput, `%util`, and latency captured under load (or N/A)
- [ ] Per-process disk I/O captured when a specific PID exists (`iotop`, `pidstat -d`)
...
- [ ] Peak CPU, RAM, GPU util, disk `%util`/throughput/latency, and temperatures recorded
- [ ] Throttling signals checked (clock drops, `nvidia-smi` perf state, thermal plateau)
- [ ] Disk saturation checked (`%util` near 100%, rising `await`, queue depth)
- [ ] Swap growth or OOM/kernel messages checked when RAM is high

## Phase 4 — Analysis

- [ ] Primary bottleneck classified (CPU / memory / disk / GPU / thermal / I/O suspicion)
